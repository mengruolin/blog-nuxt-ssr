(function webpackUniversalModuleDefinition (root, factory) {
  if (typeof exports === 'object' && typeof module === 'object') { module.exports = factory() } else if (typeof define === 'function' && define.amd) { define([], factory) } else if (typeof exports === 'object') { exports.COS = factory() } else { root.COS = factory() }
  ...
  /******/ ([
    ...
    /***/ function (module, exports, __webpack_require__) {
      ...
      const md5 = __webpack_require__(6)
      const CryptoJS = __webpack_require__(7)
      const xml2json = __webpack_require__(8)
      const json2xml = __webpack_require__(11)
      ...
      const getAuth = function (opt) {
        ...
        const headers = clone(opt.Headers || opt.headers || {})
        ...
        const authorization = ['q-sign-algorithm=' + qSignAlgorithm, 'q-ak=' + qAk, 'q-sign-time=' + qSignTime, 'q-key-time=' + qKeyTime, 'q-header-list=' + qHeaderList, 'q-url-param-list=' + qUrlParamList, 'q-signature=' + qSignature].join('&')
      ...
      const readAsBinaryString = function (blob, callback) {
        let readFun
      ...
      const fileSliceNeedCopy = (function () {
        ...
        const check = function (ua) {
          const ChromeVersion = (ua.match(/Chrome\/([.\d]+)/) || [])[1]
          const QBCoreVersion = (ua.match(/QBCore\/([.\d]+)/) || [])[1]
          const QQBrowserVersion = (ua.match(/QQBrowser\/([.\d]+)/) || [])[1]
          const need = ChromeVersion && compareVersion(ChromeVersion, '53.0.2785.116') < 0 && QBCoreVersion && compareVersion(QBCoreVersion, '3.53.991.400') < 0 && QQBrowserVersion && compareVersion(QQBrowserVersion, '9.0.2524.400') <= 0 || false
          return need
        }
      ...
      // 获取文件分片
      const fileSlice = function (file, start, end, isUseToUpload, callback) {
        let blob
        if (file.slice) {
      ...
      // 获取文件内容的 MD5
      const getBodyMd5 = function (UploadCheckContentMd5, Body, callback, onProgress) {
      ...
      // 获取文件 md5 值
      const md5ChunkSize = 1024 * 1024
      const getFileMd5 = function (blob, callback, onProgress) {
        const size = blob.size
        let loaded = 0
        let md5ctx = md5.getCtx()
        var next = function (start) {
          if (start >= size) {
            const hash = md5ctx.digest('hex')
            callback(null, hash)
            return
          }
          const end = Math.min(size, start + md5ChunkSize)
          util.fileSlice(blob, start, end, false, function (chunk) {
            readAsBinaryString(chunk, function (content) {
              ...
              content = null
              if (onProgress) { onProgress({ loaded, total: size, percent: Math.round(loaded / size * 10000) / 10000 }) }
      ...
      const hasMissingParams = function (apiName, params) {
        const Bucket = params.Bucket
        const Region = params.Region
        const Key = params.Key
        if (apiName.includes('Bucket') || apiName === 'deleteMultipleObject' || apiName === 'multipartList' || apiName === 'listObjectVersions') {
          if (!Bucket) { return 'Bucket' }
          if (!Region) { return 'Region' }
        } else if (apiName.includes('Object') || apiName.includes('multipart') || apiName === 'sliceUploadFile' || apiName === 'abortUploadTask') {
          if (!Bucket) { return 'Bucket' }
          if (!Region) { return 'Region' }
          if (!Key) { return 'Key' }
        }
      ...
      const formatParams = function (apiName, params) {
        // 复制参数对象
          ...
          if (params && typeof params === 'object') {
            (function () {
              for (const key in params) {
                if (params.hasOwnProperty(key) && key.includes('x-cos-')) {
      ...
      const apiWrapper = function (apiName, apiFn) {
        ...
        // 处理参数
          ...
          if (apiName !== 'getService' && apiName !== 'abortUploadTask') {
            ...
            if (params.Region) {
              ...
              // 判断 region 格式
              if (!this.options.CompatibilityMode && !params.Region.includes('-') && params.Region !== 'yfb' && params.Region !== 'default') {
          ...
          const res = apiFn.call(this, params, _callback)
      ...
      const throttleOnProgress = function (total, onProgress) {
        const self = this
        let size0 = 0
        let size1 = 0
        let time0 = Date.now()
        ...
        function update () {
          ...
          if (onProgress && typeof onProgress === 'function') {
            time1 = Date.now()
            const speed = Math.max(0, Math.round((size1 - size0) / ((time1 - time0) / 1000) * 100) / 100)
            let percent
            if (size1 === 0 && total === 0) {
              percent = 1
            } else {
      ...
      const getFileSize = function (api, params, callback) {
        let size
 ...
 */
      ...
      NamedNodeMap.prototype = {
        ...
        setNamedItemNS (attr) { // raises: WRONG_DOCUMENT_ERR,NO_MODIFICATION_ALLOWED_ERR,INUSE_ATTRIBUTE_ERR
          const el = attr.ownerElement; let oldAttr
          if (el && el != this._ownerElement) {
            throw new DOMException(INUSE_ATTRIBUTE_ERR)
          }
 ...
 */
    ...
    /* 2 */
    /***/ function (module, exports) {
      const initEvent = function (cos) {
        const listeners = {}
        const getList = function (action) {
        ...
        cos.emit = function (action, data) {
          const list = getList(action).map(function (cb) {
            return cb
          })
    ...
    /***/ function (module, exports, __webpack_require__) {
      ...
      // 按照文件特征值，缓存 UploadId
      const cacheKey = 'cos_sdk_upload_cache'
      ...
      var mod = {
        ...
        // 用上传参数生成哈希值
        getFileId (file, ChunkSize, Bucket, Key) {
        ...
        // 缓存 UploadId
        saveUploadId (uuid, UploadId, limit) {
          init()
          if (!uuid) { return }
          // 清理没用的 UploadId
          for (let i = cache.length - 1; i >= 0; i--) {
    ...
    /* 4 */
    /***/ function (module, exports, __webpack_require__) {
      const COS = __webpack_require__(5)
      module.exports = COS
      /***/ },
    /* 5 */
    /***/ function (module, exports, __webpack_require__) {
      ...
      const advance = __webpack_require__(15)
 ...
 */
      var CryptoJS = CryptoJS || (function (g, l) {
        ...
        const d = e.lib = {}
        const m = function () {}
        const k = d.Base = { extend (a) {
          m.prototype = this; const c = new m(); a && c.mixIn(a); c.hasOwnProperty('init') || (c.init = function () {
            c.$super.init.apply(this, arguments)
          }); c.init.prototype = c; c.$super = this; return c
        },
        create () {
          const a = this.extend(); a.init.apply(a, arguments); return a
        },
        init () {},
        mixIn (a) {
          for (const c in a) { a.hasOwnProperty(c) && (this[c] = a[c]) }a.hasOwnProperty('toString') && (this.toString = a.toString)
        },
        clone () {
          return this.init.prototype.extend(this)
        } }
        var p = d.WordArray = k.extend({ init (a, c) {
        ...
        concat (a) {
          const c = this.words
          const q = a.words
          const f = this.sigBytes; a = a.sigBytes; this.clamp(); if (f % 4) { for (var b = 0; b < a; b++) { c[f + b >>> 2] |= (q[b >>> 2] >>> 24 - 8 * (b % 4) & 255) << 24 - 8 * ((f + b) % 4) } } else if (q.length > 65535) { for (b = 0; b < a; b += 4) { c[f + b >>> 2] = q[b >>> 2] } } else { c.push.apply(c, q) } this.sigBytes += a; return this
        },
        clamp () {
          const a = this.words
          const c = this.sigBytes; a[c >>> 2] &= 4294967295 << 32 - 8 * (c % 4); a.length = g.ceil(c / 4)
        },
        clone () {
          const a = k.clone.call(this); a.words = this.words.slice(0); return a
        },
        random (a) {
          for (var c = [], b = 0; b < a; b += 4) { c.push(4294967296 * g.random() | 0) } return new p.init(c, a)
        } })
        const b = e.enc = {}
        var n = b.Hex = { stringify (a) {
          const c = a.words; a = a.sigBytes; for (var b = [], f = 0; f < a; f++) {
            const d = c[f >>> 2] >>> 24 - 8 * (f % 4) & 255; b.push((d >>> 4).toString(16)); b.push((d & 15).toString(16))
          } return b.join('')
        },
        parse (a) {
          for (var c = a.length, b = [], f = 0; f < c; f += 2) { b[f >>> 3] |= parseInt(a.substr(f, 2), 16) << 24 - 4 * (f % 8) } return new p.init(b, c / 2)
        } }
        const j = b.Latin1 = { stringify (a) {
          const c = a.words; a = a.sigBytes; for (var b = [], f = 0; f < a; f++) { b.push(String.fromCharCode(c[f >>> 2] >>> 24 - 8 * (f % 4) & 255)) } return b.join('')
        },
        parse (a) {
          for (var c = a.length, b = [], f = 0; f < c; f++) { b[f >>> 2] |= (a.charCodeAt(f) & 255) << 24 - 8 * (f % 4) } return new p.init(b, c)
        } }
        const h = b.Utf8 = { stringify (a) {
          try {
        ...
        const r = d.BufferedBlockAlgorithm = k.extend({ reset () {
        ...
        _process (a) {
          ...
          var e = f / (4 * d)
          var e = a ? g.ceil(e) : g.max((e | 0) - this._minBufferSize, 0); a = e * d; f = g.min(4 * a, f); if (a) {
        ...
        _minBufferSize: 0 }); d.Hasher = r.extend({ cfg: k.extend(),
          init (a) {
      ...
      (function () {
        ...
        const d = l.Hasher
        const m = []
        var l = g.algo.SHA1 = d.extend({ _doReset () {
          this._hash = new e.init([1732584193, 4023233417, 2562383102, 271733878, 3285377520])
        },
        _doProcessBlock (d, e) {
          for (var b = this._hash.words, n = b[0], j = b[1], h = b[2], g = b[3], l = b[4], a = 0; a < 80; a++) {
            if (a < 16) { m[a] = d[e + a] | 0 } else {
              var c = m[a - 3] ^ m[a - 8] ^ m[a - 14] ^ m[a - 16]; m[a] = c << 1 | c >>> 31
            }c = (n << 5 | n >>> 27) + l + m[a]; c = a < 20 ? c + ((j & h | ~j & g) + 1518500249) : a < 40 ? c + ((j ^ h ^ g) + 1859775393) : a < 60 ? c + ((j & h | j & g | h & g) - 1894007588) : c + ((j ^ h ^ g) - 899497514); l = g; g = h; h = j << 30 | j >>> 2; j = n; n = c
        ...
        _doFinalize () {
          ...
          const g = 8 * d.sigBytes; e[g >>> 5] |= 128 << 24 - g % 32; e[(g + 64 >>> 9 << 4) + 14] = Math.floor(b / 4294967296); e[(g + 64 >>> 9 << 4) + 15] = b; d.sigBytes = 4 * e.length; this._process(); return this._hash
        },
        clone () {
          const e = d.clone.call(this); e._hash = this._hash.clone(); return e
        } }); g.SHA1 = d._createHelper(l); g.HmacSHA1 = d._createHmacHelper(l)
      })();
      (function () {
        const g = CryptoJS
        const l = g.enc.Utf8; g.algo.HMAC = g.lib.Base.extend({ init (e, d) {
          e = this._hasher = new e.init(); typeof d === 'string' && (d = l.parse(d)); const g = e.blockSize
          const k = 4 * g; d.sigBytes > k && (d = e.finalize(d)); d.clamp(); for (var p = this._oKey = d.clone(), b = this._iKey = d.clone(), n = p.words, j = b.words, h = 0; h < g; h++) { n[h] ^= 1549556828, j[h] ^= 909522486 }p.sigBytes = b.sigBytes = k; this.reset()
        },
        reset () {
          const e = this._hasher; e.reset(); e.update(this._iKey)
        },
        update (e) {
          this._hasher.update(e); return this
        },
        finalize (e) {
          const d = this._hasher; e = d.finalize(e); d.reset(); return d.finalize(this._oKey.clone().concat(e))
        } })
      })();
 ...
 */
      const DOMParser = __webpack_require__(9).DOMParser
      ...
      var xmlToJSON = (function () {
        ...
        // initialize options
          ...
          const childNum = oXMLParent.childNodes.length
          if (childNum) {
        ...
        this.xmlToString = function (xmlDoc) {
          try {
            const xmlString = xmlDoc.xml ? xmlDoc.xml : new XMLSerializer().serializeToString(xmlDoc)
            return xmlString
          } catch (err) {
            return null
          }
      ...
      const xml2json = function (xmlString) {
 ...
 */
    ...
    /***/ function (module, exports) {
      ...
      const element_start_char = 'a-zA-Z_\u00C0-\u00D6\u00D8-\u00F6\u00F8-\u00FF\u0370-\u037D\u037F-\u1FFF\u200C-\u200D\u2070-\u218F\u2C00-\u2FFF\u3001-\uD7FF\uF900-\uFDCF\uFDF0-\uFFFD'
      ...
      const process_to_xml = function (node_data, options) {
        ...
        return (function fn (node_data, node_descriptor, level) {
          ...
          if (Array.isArray ? Array.isArray(node_data) : Array.isArray(node_data)) {
            type = 'array'
          ...
          switch (type) {
            ...
            case 'object':
              var nodes = []
              for (const name in node_data) {
                if (node_data.hasOwnProperty(name)) {
                  if (Array.isArray(node_data[name])) {
                    for (let j = 0; j < node_data[name].length; j++) {
      ...
      const xml_header = function (standalone) {
      ...
      module.exports = function (obj, options) {
        ...
        const ret = [xmlheader, options.prettyPrint && docType ? '\n' : '', docType, process_to_xml(obj, options)]
    ...
    /***/ function (module, exports, __webpack_require__) {
      ...
      const originApiMap = {}
      const transferToTaskMethod = function (apiMap, apiName) {
      ...
      const initTask = function (cos) {
        ...
        const clearQueue = function () {
          ...
          for (let i = 0; i < nextUploadIndex && // 小于当前操作的 index 才清理
        i < queue.length && // 大于队列才清理
        queue.length > cos.options.UploadQueueSize // 如果还太多，才继续清理
            ;) {
            const isActive = queue[i].state === 'waiting' || queue[i].state === 'checking' || queue[i].state === 'uploading'
            if (!queue[i] || !isActive) {
        ...
        // 检查是否允许增加执行进程
          ...
          // 上传该遍历到的任务
          const task = queue[nextUploadIndex]
          nextUploadIndex++
          uploadingFileCount++
          task.state = 'checking'
          task.params.onTaskStart && task.params.onTaskStart(formatTask(task))
          !task.params.UploadData && (task.params.UploadData = {})
          const apiParams = util.formatParams(task.api, task.params)
        ...
        const killTask = function (id, switchToState) {
          const task = tasks[id]
          if (!task) { return }
          const waiting = task && task.state === 'waiting'
          const running = task && (task.state === 'checking' || task.state === 'uploading')
    ...
    /* 13 */
    /***/ function (module, exports, __webpack_require__) {
      const REQUEST = __webpack_require__(14)
      const util = __webpack_require__(0)
 ...
 */
      function getBucket (params, callback) {
        ...
        submitRequest.call(this, {
          ...
          const ListBucketResult = data.ListBucketResult || {}
          let Contents = ListBucketResult.Contents || []
          let CommonPrefixes = ListBucketResult.CommonPrefixes || []

          Contents = util.isArray(Contents) ? Contents : [Contents]
          CommonPrefixes = util.isArray(CommonPrefixes) ? CommonPrefixes : [CommonPrefixes]

          const result = util.clone(ListBucketResult)
          util.extend(result, {
            Contents,
            CommonPrefixes,
 ...
 */
      function putBucketAcl (params, callback) {
        ...
        if (params.AccessControlPolicy) {
          const AccessControlPolicy = util.clone(params.AccessControlPolicy || {})
          let Grants = AccessControlPolicy.Grants || AccessControlPolicy.Grant
 ...
 */
      function getBucketAcl (params, callback) {
        submitRequest.call(this, {
          ...
          const AccessControlPolicy = data.AccessControlPolicy || {}
          const Owner = AccessControlPolicy.Owner || {}
          let Grant = AccessControlPolicy.AccessControlList.Grant || []
          Grant = util.isArray(Grant) ? Grant : [Grant]
          let result = decodeAcl(AccessControlPolicy)
 ...
 */
      function putBucketCors (params, callback) {
        const CORSConfiguration = params.CORSConfiguration || {}
        let CORSRules = CORSConfiguration.CORSRules || params.CORSRules || []
        CORSRules = util.clone(util.isArray(CORSRules) ? CORSRules : [CORSRules])
        util.each(CORSRules, function (rule) {
          util.each(['AllowedOrigin', 'AllowedHeader', 'AllowedMethod', 'ExposeHeader'], function (key, k) {
            const sKey = key + 's'
            const val = rule[sKey] || rule[key] || []
            delete rule[sKey]
            rule[key] = util.isArray(val) ? val : [val]
          })
        })

        const xml = util.json2xml({ CORSConfiguration: { CORSRule: CORSRules } })
 ...
 */
      function getBucketCors (params, callback) {
        submitRequest.call(this, {
          ...
          const CORSConfiguration = data.CORSConfiguration || {}
          let CORSRules = CORSConfiguration.CORSRules || CORSConfiguration.CORSRule || []
          CORSRules = util.clone(util.isArray(CORSRules) ? CORSRules : [CORSRules])
 ...
 */
      function putBucketTagging (params, callback) {
        ...
        let Tags = Tagging.TagSet || Tagging.Tags || params.Tags || []
        Tags = util.clone(util.isArray(Tags) ? Tags : [Tags])
        const xml = util.json2xml({ Tagging: { TagSet: { Tag: Tags } } })
 ...
 */
      ...
      function putBucketLifecycle (params, callback) {
        const LifecycleConfiguration = params.LifecycleConfiguration || {}
        let Rules = LifecycleConfiguration.Rules || params.Rules || []
        Rules = util.clone(Rules)
        const xml = util.json2xml({ LifecycleConfiguration: { Rule: Rules } })
      ...
      function putBucketVersioning (params, callback) {
        ...
        const VersioningConfiguration = params.VersioningConfiguration || {}
        const xml = util.json2xml({ VersioningConfiguration })
      ...
      function putBucketReplication (params, callback) {
        const ReplicationConfiguration = util.clone(params.ReplicationConfiguration)
        let xml = util.json2xml({ ReplicationConfiguration })
 ...
 */
      function putBucketWebsite (params, callback) {
        ...
        const WebsiteConfiguration = util.clone(params.WebsiteConfiguration || {})
        let RoutingRules = WebsiteConfiguration.RoutingRules || WebsiteConfiguration.RoutingRule || []
        RoutingRules = util.isArray(RoutingRules) ? RoutingRules : [RoutingRules]
        delete WebsiteConfiguration.RoutingRule
        delete WebsiteConfiguration.RoutingRules
        RoutingRules.length > 0 && (WebsiteConfiguration.RoutingRules = { RoutingRule: RoutingRules })
        const xml = util.json2xml({ WebsiteConfiguration })
 ...
 */
      function getBucketWebsite (params, callback) {
        submitRequest.call(this, {
          ...
          const WebsiteConfiguration = data.WebsiteConfiguration || {}
          if (WebsiteConfiguration.RoutingRules) {
            let RoutingRules = util.clone(WebsiteConfiguration.RoutingRules.RoutingRule || [])
 ...
 */
      ...
      function listObjectVersions (params, callback) {
        ...
        submitRequest.call(this, {
          ...
          const ListVersionsResult = data.ListVersionsResult || {}
          let DeleteMarkers = ListVersionsResult.DeleteMarker || []
          DeleteMarkers = util.isArray(DeleteMarkers) ? DeleteMarkers : [DeleteMarkers]
          let Versions = ListVersionsResult.Version || []
          Versions = util.isArray(Versions) ? Versions : [Versions]

          const result = util.clone(ListVersionsResult)
 ...
 */
      function putObject (params, callback) {
        ...
        const onProgress = util.throttleOnProgress.call(self, FileSize, params.onProgress)
        ...
        const ContentType = headers['Content-Type'] || params.Body && params.Body.type
        !headers['Content-Type'] && ContentType && (headers['Content-Type'] = ContentType)

        const needCalcMd5 = params.UploadAddMetaMd5 || self.options.UploadAddMetaMd5 || self.options.UploadCheckContentMd5
 ...
 */
      function getObjectAcl (params, callback) {
        submitRequest.call(this, {
          ...
          const AccessControlPolicy = data.AccessControlPolicy || {}
          const Owner = AccessControlPolicy.Owner || {}
          let Grant = AccessControlPolicy.AccessControlList && AccessControlPolicy.AccessControlList.Grant || []
          Grant = util.isArray(Grant) ? Grant : [Grant]
          let result = decodeAcl(AccessControlPolicy)
          if (data.headers && data.headers['x-cos-acl']) {
            result.ACL = data.headers['x-cos-acl']
 ...
 */
      function putObjectCopy (params, callback) {
        ...
        const m = CopySource.match(/^([^.]+-\d+)\.cos(v6)?\.([^.]+)\.[^/]+\/(.+)$/)
        if (!m) {
          callback({ error: 'CopySource format error' })
          return
        }

        const SourceBucket = m[1]
        const SourceRegion = m[3]
        const SourceKey = decodeURIComponent(m[4])
        ...
        submitRequest.call(this, {
          ...
          const result = util.clone(data.CopyObjectResult || {})
          util.extend(result, {
            statusCode: data.statusCode,
            headers: data.headers
          })
      ...
      function uploadPartCopy (params, callback) {
        ...
        submitRequest.call(this, {
          ...
          const result = util.clone(data.CopyPartResult || {})
          util.extend(result, {
            statusCode: data.statusCode,
            headers: data.headers
          })
      ...
      function deleteMultipleObject (params, callback) {
        ...
        const xml = util.json2xml({ Delete: { Object: Objects, Quiet: Quiet || false } })

        const headers = params.Headers
        headers['Content-Type'] = 'application/xml'
        headers['Content-MD5'] = util.binaryBase64(util.md5(xml))

        const Scope = util.map(Objects, function (v) {
          return {
            action: 'name/cos:DeleteObject',
            bucket: params.Bucket,
            region: params.Region,
        ...
        submitRequest.call(this, {
          ...
          const DeleteResult = data.DeleteResult || {}
          let Deleted = DeleteResult.Deleted || []
          let Errors = DeleteResult.Error || []

          Deleted = util.isArray(Deleted) ? Deleted : [Deleted]
          Errors = util.isArray(Errors) ? Errors : [Errors]

          const result = util.clone(DeleteResult)
          util.extend(result, {
            Error: Errors,
            Deleted,
            statusCode: data.statusCode,
      ...
      function restoreObject (params, callback) {
        ...
        const RestoreRequest = params.RestoreRequest || {}
        const xml = util.json2xml({ RestoreRequest })
 ...
 */
      function multipartComplete (params, callback) {
        ...
        const xml = util.json2xml({ CompleteMultipartUpload: { Part: Parts } })
        ...
        submitRequest.call(this, {
          ...
          const CompleteMultipartUploadResult = data.CompleteMultipartUploadResult || {}
          const result = util.extend(CompleteMultipartUploadResult, {
            Location: url,
            statusCode: data.statusCode,
            headers: data.headers
          })
 ...
 */
      function multipartList (params, callback) {
        ...
        submitRequest.call(this, {
          ...
          const result = util.clone(data.ListMultipartUploadsResult || {})
          util.extend(result, {
            statusCode: data.statusCode,
            headers: data.headers
          })
 ...
 */
      function multipartListPart (params, callback) {
        ...
        submitRequest.call(this, {
          ...
          const ListPartsResult = data.ListPartsResult || {}
          let Part = ListPartsResult.Part || []
          Part = util.isArray(Part) ? Part : [Part]

          ListPartsResult.Part = Part
          const result = util.clone(ListPartsResult)
          util.extend(result, {
            statusCode: data.statusCode,
            headers: data.headers
          })
 ...
 */
      function getObjectUrl (params, callback) {
        ...
        const AuthData = getAuthorizationAsync.call(this, {
          ...
          let signUrl = url
          signUrl += '?' + (AuthData.Authorization.includes('q-signature') ? AuthData.Authorization : 'sign=' + encodeURIComponent(AuthData.Authorization))
 ...
 */
      function decodeAcl (AccessControlPolicy) {
        ...
        const PublicAcl = { READ: 0, WRITE: 0, FULL_CONTROL: 0 }
      ...
      function getUrl (params) {
        const longBucket = params.bucket
        const shortBucket = longBucket.substr(0, longBucket.lastIndexOf('-'))
        const appId = longBucket.substr(longBucket.lastIndexOf('-') + 1)
        let domain = params.domain
        const region = params.region
        const object = params.object
        const protocol = params.protocol || (util.isBrowser && location.protocol === 'http:' ? 'http:' : 'https:')
        if (!domain) {
          if (['cn-south', 'cn-south-2', 'cn-north', 'cn-east', 'cn-southwest', 'sg'].includes(region)) {
      ...
      function getAuthorizationAsync (params, callback) {
        const headers = util.clone(params.Headers)
        ...
        const cb = function (AuthData) {
        // 检查签名格式
          let formatAllow = false
          let auth = AuthData.Authorization
          if (auth) {
            if (auth.includes(' ')) {
              formatAllow = false
            } else if (auth.includes('q-sign-algorithm=') && auth.includes('q-ak=') && auth.includes('q-sign-time=') && auth.includes('q-key-time=') && auth.includes('q-url-param-list=')) {
              formatAllow = true
            } else {
              try {
                auth = atob(auth)
                if (auth.includes('a=') && auth.includes('k=') && auth.includes('t=') && auth.includes('r=') && auth.includes('b=')) {
                  formatAllow = true
                }
              } catch (e) {}
            }
        ...
        // PathName
        let KeyName = params.Key || ''
        if (self.options.ForcePathStyle && Bucket) {
          KeyName = Bucket + '/' + KeyName
        }
        const Pathname = '/' + KeyName
        ...
        if (!Scope) {
          ...
          const ResourceKey = params.ResourceKey || params.Key || ''
          Scope = params.Scope || [{
            action: Action,
            bucket: Bucket,
            region: Region,
        ...
        const ScopeKey = util.md5(JSON.stringify(Scope))
        ...
        (function () {
          ...
          for (i = self._StsCache.length - 1; i >= 0; i--) {
            ...
            const compareTime = Math.round(util.getSkewTime(self.options.SystemClockOffset) / 1000) + 30
        ...
        const calcAuthByTmpKey = function () {
          const KeyTime = StsData.StartTime && StsData.ExpiredTime ? StsData.StartTime + ';' + StsData.ExpiredTime : ''
          const Authorization = util.getAuth({
      ...
      function submitRequest (params, callback) {
        ...
        const Query = util.clone(params.qs)
        params.action && (Query[params.action] = '')

        var next = function (tryTimes) {
          const oldClockOffset = self.options.SystemClockOffset
      ...
      function _submitRequest (params, callback) {
        ...
        if (params.onProgress && typeof params.onProgress === 'function') {
          const contentLength = body && (body.size || body.length) || 0
        ...
        var sender = REQUEST(opt, function (err, response, body) {
          ...
          if (rawBody) {
            ...
            try {
              jsonRes = body && body.includes('<') && body.includes('>') && util.xml2json(body) || {}
            } catch (e) {
              jsonRes = body || {}
            }
          }
          ...
          const statusSuccess = Math.floor(statusCode / 100) === 2 // 200 202 204 206
          if (!statusSuccess) {
      ...
      const API_MAP = {
        ...
        putBucketPolicy,
        deleteBucketPolicy,
        putBucketTagging, // BucketTagging
        getBucketTagging,
        deleteBucketTagging,
        putBucketLifecycle, // BucketLifecycle
        getBucketLifecycle,
        deleteBucketLifecycle,
        putBucketVersioning, // BucketVersioning
        getBucketVersioning,
      ...
      function warnOldApi (apiName, fn, proto) {
        util.each(['Cors', 'Acl'], function (suffix) {
          if (apiName.slice(-suffix.length) === suffix) {
            const oldName = apiName.slice(0, -suffix.length) + suffix.toUpperCase()
            const apiFn = util.apiWrapper(apiName, fn)
            let warned = false
    ...
    /***/ function (module, exports) {
      const $ = (function () {
        ...
        const version = '1.11.1 -css,-css/addGetHookIf,-css/curCSS,-css/defaultDisplay,-css/hiddenVisibleSelectors,-css/support,-css/swap,-css/var/cssExpand,-css/var/isHidden,-css/var/rmargin,-css/var/rnumnonpx,-effects,-effects/Tween,-effects/animatedSelector,-effects/support,-dimensions,-offset,-deprecated,-event-alias,-wrap'
     ...
     */
        ...
        jQuery.extend({
          ...
          ajax (url, options) {
            ...
            const globalEventContext = s.context && (callbackContext.nodeType || callbackContext.jquery) ? jQuery(callbackContext) : jQuery.event
        ...
        jQuery.ajaxPrefilter('json jsonp', function (s, originalSettings, jqXHR) {
          ...
          const jsonProp = s.jsonp !== false && (rjsonp.test(s.url) ? 'url' : typeof s.data === 'string' && !(s.contentType || '').indexOf('application/x-www-form-urlencoded') && rjsonp.test(s.data) && 'data')
      ...
      const request = function (options, callback) {
        ...
        // qs
        if (options.qs) {
          const qsStr = queryStringify(options.qs)
          if (qsStr) {
            options.url += (!options.url.includes('?') ? '?' : '&') + qsStr
          }
          delete options.qs
        }
    ...
    /***/ function (module, exports, __webpack_require__) {
      ...
      const EventProxy = __webpack_require__(2).EventProxy
      const util = __webpack_require__(0)
      ...
      function sliceUploadFile (params, callback) {
        ...
        const Key = params.Key
        const Body = params.Body
        let ChunkSize = params.ChunkSize || params.SliceSize || self.options.ChunkSize
        const AsyncLimit = params.AsyncLimit
        const StorageClass = params.StorageClass || 'Standard'
        const ServerSideEncryption = params.ServerSideEncryption
        let FileSize
        ...
        // 处理 UploadId 缓存
          const uuid = session.getFileId(Body, params.ChunkSize, Bucket, Key)
      ...
      function getUploadIdAndPartList (params, callback) {
        ...
        let FinishSliceCount = 0
        let FinishSize = 0
        const onHashProgress = util.throttleOnProgress.call(self, FileSize, params.onHashProgress)
        const getChunkETag = function (PartNumber, callback) {
        ...
        const isAvailableUploadList = function (PartList, callback) {
          ...
          // 检查分片大小
          if (PartCount > 1) {
            const PartSliceSize = Math.max(PartList[0].Size, PartList[1].Size)
        ...
        // 转换成 map
          ...
          for (let PartNumber = 1; PartNumber <= SliceCount; PartNumber++) {
            let item = map[PartNumber]
            if (item) {
        ...
        ep.on('no_available_upload_id', function () {
          ...
          // 获取 File 或 Blob 的 type 属性，如果有，作为文件 Content-Type
          const ContentType = params.Headers['Content-Type'] || params.Body && params.Body.type
          if (ContentType) {
        ...
        // 串行地，找一个内容一致的 UploadId
          ...
          Async.eachLimit(UploadIdList, 1, function (UploadId, asyncCallback) {
            ...
            wholeMultipartListPart.call(self, {
              ...
              const PartList = PartListData.PartList
        ...
        // 在本地找可用的 UploadId
          const uuid = session.getFileId(params.Body, params.ChunkSize, Bucket, Key)
          const LocalUploadIdList = session.getUploadIdList(uuid)
          ...
          var next = function (index) {
            ...
            const UploadId = LocalUploadIdList[index]
        ...
        // 获取符合条件的 UploadId 列表，因为同一个文件可以有多个上传任务。
          wholeMultipartList.call(self, {
            ...
            // 整理远端 UploadId 列表
            const RemoteUploadIdList = util.filter(data.UploadList, function (item) {
 ...
 */
      function uploadSliceList (params, cb) {
        ...
        const FileSize = params.FileSize
        const SliceSize = params.SliceSize
        const ChunkParallel = Math.min(params.AsyncLimit || self.options.ChunkParallelLimit || 1, 256)
        const Body = params.Body
        const SliceCount = Math.ceil(FileSize / SliceSize)
        let FinishSize = 0
        const ServerSideEncryption = params.ServerSideEncryption
        const needUploadSlices = util.filter(UploadData.PartList, function (SliceItem) {
        ...
        Async.eachLimit(needUploadSlices, ChunkParallel, function (SliceItem, asyncCallback) {
          ...
          const currentSize = Math.min(FileSize, SliceItem.PartNumber * SliceSize) - (SliceItem.PartNumber - 1) * SliceSize
          let preAddSize = 0
      ...
      function uploadSliceItem (params, callback) {
        ...
        const Key = params.Key
        const FileSize = params.FileSize
        const FileBody = params.Body
        const PartNumber = params.PartNumber * 1
        const SliceSize = params.SliceSize
        const ServerSideEncryption = params.ServerSideEncryption
        const UploadData = params.UploadData
        const ChunkRetryTimes = self.options.ChunkRetryTimes + 1
        ...
        const PartItem = UploadData.PartList[PartNumber - 1]
      ...
      function uploadSliceComplete (params, callback) {
        ...
        const self = this
        const ChunkRetryTimes = this.options.ChunkRetryTimes + 1
        const Parts = SliceList.map(function (item) {
          return {
 ...
 */
      ...
      function abortUploadTaskArray (params, callback) {
        ...
        let index = 0
        const resultList = new Array(AbortArray.length)
        Async.eachLimit(AbortArray, AsyncLimit, function (AbortItem, callback) {
          ...
          const UploadId = AbortItem.UploadId || AbortItem.UploadID
      ...
      function uploadFiles (params, callback) {
        ...
        // 判断多大的文件使用分片上传
        const SliceSize = params.SliceSize === undefined ? self.options.SliceSize : params.SliceSize

        // 汇总返回进度
        let TotalSize = 0
        let TotalFinish = 0
        const onTotalProgress = util.throttleOnProgress.call(self, TotalFinish, params.onProgress)

        // 汇总返回回调
        let unFinishCount = params.files.length
        const _onTotalFileFinish = params.onFileFinish
        const resultList = Array(unFinishCount)
        const onTotalFileFinish = function (err, data, options) {
        ...
        util.each(params.files, function (fileParams, index) {
          (function () {
            ...
            const Body = fileParams.Body
            const FileSize = Body.size || Body.length || 0
            const fileInfo = { Index: index, TaskId: '' }
            ...
            // 处理单个文件 TaskReady
            const _onTaskReady = fileParams.onTaskReady
            const onTaskReady = function (tid) {
              fileInfo.TaskId = tid
            ...
            // 处理单个文件进度
            let PreAddSize = 0
            const _onProgress = fileParams.onProgress
            ...
            // 处理单个文件完成
            const _onFileFinish = fileParams.onFileFinish
            const onFileFinish = function (err, data) {
              _onFileFinish && _onFileFinish(err, data)
              onTotalFileFinish && onTotalFileFinish(err, data, fileInfo)
            }

            // 添加上传任务
            const api = FileSize >= SliceSize ? 'sliceUploadFile' : 'putObject'
            taskList.push({
              api,
              params: fileParams,
      ...
      function sliceCopyFile (params, callback) {
        ...
        const SourceRegion = m[3]
        const SourceKey = decodeURIComponent(m[4])
        let CopySliceSize = params.CopySliceSize === undefined ? self.options.CopySliceSize : params.CopySliceSize
        CopySliceSize = Math.max(0, CopySliceSize)

        let ChunkSize = params.CopyChunkSize || this.options.CopyChunkSize
        const ChunkParallel = this.options.CopyChunkParallelLimit
        ...
        ep.on('get_copy_data_finish', function (UploadData) {
          Async.eachLimit(UploadData.PartList, ChunkParallel, function (SliceItem, asyncCallback) {
            const PartNumber = SliceItem.PartNumber
            const CopySourceRange = SliceItem.CopySourceRange
            const currentSize = SliceItem.end - SliceItem.start
            let preAddSize = 0
        ...
        // 控制分片大小
          (function () {
            ...
            for (let partNumber = 1; partNumber <= ChunkCount; partNumber++) {
              ...
              const end = partNumber * ChunkSize < FileSize ? partNumber * ChunkSize - 1 : FileSize - 1
              const item = {
         ...
         */
          if (SourceHeaders['x-cos-storage-class'] === 'ARCHIVE') {
            const restoreHeader = SourceHeaders['x-cos-restore']
      ...
      function copySliceItem (params, callback) {
        ...
        const UploadId = params.UploadId
        const PartNumber = params.PartNumber * 1
        const CopySourceRange = params.CopySourceRange
    ...
    /* 16 */
    /***/ function (module, exports) {
      const eachLimit = function (arr, limit, iterator, callback) {
      ...
      const retry = function (times, iterator, callback) {
        var next = function (index) {
